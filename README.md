...
ローカルでは `.env.example` を参考に `.env` を設定してください。GitHub Actionsでは、リポジトリの `Settings > Secrets and variables > Actions` に `LINE_CHANNEL_ACCESS_TOKEN` という名前で登録します。トークンをコードやワークフローへ直接記載しないでください。
